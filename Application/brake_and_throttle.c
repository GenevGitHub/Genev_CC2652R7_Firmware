...
static void throttleSpeedControlAlgorithm()
{
    ...
    else
    {
        ...
        {
            if (((float)(throttleSpeedControl - RPM_temp) * 100 / allowableRPM) <= 2)  // difference in rpm less than "x" %
            {
                alpha = (float) 0;
            }
            else if ((((float)(throttleSpeedControl - RPM_temp) * 100 / allowableRPM) > 2) && (((float)(throttleSpeedControl - RPM_temp) * 100 / allowableRPM) < 10))
            {
                alpha = ((float)(throttleSpeedControl - RPM_temp) / allowableRPM);
            }
            else {
                alpha = (float)(10) / (allowableRPM);
            }
        ...
        {
            if (((float)(RPM_temp - throttleSpeedControl) * 100 / allowableRPM) <= 2)  // difference in rpm less than "x" rpm (8 rpm = 0.3 km/h)
            {
                alpha = (float) 0;
            }
            else if ((((float)(RPM_temp - throttleSpeedControl) * 100 / allowableRPM) > 2) && (((float)(RPM_temp - throttleSpeedControl) * 100 / allowableRPM) < 10))
            {
                alpha = ((float)(throttleSpeedControl - RPM_temp) / allowableRPM);
                if (alpha < -1)
                {
